...
describe('login', () => {
	...
	it('Ошибка при несуществующем email', async () => {
		...
		await expect(userService.login('wrong@example.com', 'pass', '127.0.0.1', 'UA', uid)).rejects.toThrow(ApiError)
...
describe('confirmNewDevice', () => {
	...
	it('Ошибка при неверном коде', async () => {
		...
		await expect(userService.confirmNewDevice('RIGHT', 'token', 'uid', 'ip', 'agent')).rejects.toThrow(ApiError)
	})
...
describe('changeEmail', () => {
	...
	it('Ошибка при неверном текущем email', async () => {
		pool.query.mockResolvedValueOnce({ rows: [{ email: 'wrong@example.com' }] })

		await expect(userService.changeEmail('u1', 'old@example.com', 'new@example.com')).rejects.toThrow(ApiError)
	})

	it('Ошибка если новый email уже существует', async () => {
		pool.query
			.mockResolvedValueOnce({ rows: [{ email: 'old@example.com' }] }) // текущий email совпадает
			.mockResolvedValueOnce({ rows: [{ id: 'user-2' }] }) // новый email занят

		await expect(userService.changeEmail('u1', 'old@example.com', 'old@example.com')).rejects.toThrow(ApiError)
	})
...
describe('getProfile', () => {
	...
	it('Возвращает профиль студента с оценками', async () => {
		pool.query
			.mockResolvedValueOnce({ rows: [{ email: 'stud@mail.ru', role: 'student', first_name: 'Иван', middle_name: 'Ив.', last_name: 'Иванов', student_group_name: 'Группа 1' }] }) // student info
			.mockResolvedValueOnce({ rows: [{ name: 'Математика' }, { name: 'Физика' }] }) // subjects
			.mockResolvedValueOnce({ rows: [ { subject: 'Математика', grade: 5 }, { subject: 'Математика', grade: 4 }, { subject: 'Физика', grade: null } ] }) // grades
	...
	it('Возвращает профиль преподавателя', async () => {
		pool.query
			...
			.mockResolvedValueOnce({ rows: [{ id: 10, name: 'Группа 1' }] }) // available groups
			.mockResolvedValueOnce({ rows: [{ id: 10, name: 'Группа 1' }] }) // selected groups
		...
		expect(result).toEqual({
			...
			availableGroups: [{ id: 10, name: 'Группа 1' }],
			selectedGroups: [{ id: 10, name: 'Группа 1' }]
		})
	})
...
describe('searchUsers', () => {
	...
	it('Успешно возвращает список пользователей по совпадению email', async () => {
		...
		const result = await userService.searchUsers('student@example.com')

		expect(pool.query).toHaveBeenCalledWith(expect.stringContaining('ILIKE'), ['%student@example.com%'])
		expect(result).toEqual(mockUsers)
	})
...
it('Ошибка при запросе к БД', async () => {
	...
	await expect(userService.searchUsers('тест')).rejects.toThrow('Внутренняя ошибка сервера') // ← просто строка
})
...
describe('updateUser', () => {
  ...
  const student = { rows: [{ role: 'student', email: 'stu@e.com' }] }
  const lecturer = { rows: [{ role: 'lecturer', email: 'lect@e.com' }] }
  const admin    = { rows: [{ role: 'admin',    email: 'adm@e.com' }] }
  ...
  it('Успешно обновляет email студента', async () => {
    ...
    await userService.updateUser('1', { email: 'new@e.com' }, '1')

    expect(pool.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE users SET'), expect.any(Array))
    expect(mailService.sendChangeEmail).toHaveBeenCalledWith('stu@e.com', 'new@e.com', true)
  ...
  it('Успешно сбрасывает пароль студента', async () => {
    ...
    expect(mailService.sendChangePassword).toHaveBeenCalledWith(
      'stu@e.com', 'stu@e.com', expect.any(String), true
    )
    expect(pool.query).toHaveBeenCalledWith('COMMIT')
  })
...
describe('deleteUser', () => {
  ...
  it('Ошибка при запросе к БД', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Ошибка при удалении пользователя: DB error'))
  })
})
...
describe('updateSubjects', () => {
 ...
 it('Ошибка если предмет не найден', async () => {
  ...
  it('Вызывает ROLLBACK при ошибке БД', async () => {
    ...
    pool.query
      .mockResolvedValueOnce({})              // BEGIN
      .mockRejectedValueOnce(new Error('DB error'))  // Ошибка при DELETE
